...
use gstreamer::{gst_element_error, prelude::*};
use rand::prelude::*;
...
impl App {
    ...
    > {
        ...
        let pipeline = gstreamer::parse_launch(
            "webrtcbin bundle-policy=max-bundle name=sendrecv \
             stun-server=stun://stun.l.google.com:19302 videotestsrc is-live=true pattern=ball ! \
             videoconvert ! queue ! vp8enc deadline=1 ! rtpvp8pay ! queue ! \
             application/x-rtp,media=video,encoding-name=VP8,payload=96 ! sendrecv. audiotestsrc \
             is-live=true wave=red-noise ! audioconvert ! audioresample ! queue ! opusenc ! \
             rtpopuspay ! queue ! application/x-rtp,media=audio,encoding-name=OPUS,payload=97 ! \
             sendrecv. ",
        )?;
    ...
    fn handle_pipeline_message(&self, message: &gstreamer::Message) -> Result<(), anyhow::Error> {
    ...
    fn on_answer_created(
        ...
        self.send_msg_tx
            ...
            .unwrap()
            .unbounded_send(WsMessage::Text(message))
            .with_context(|| "Failed to send SDP answer".to_string())?;
    ...
    fn handle_sdp(&self, type_: &str, sdp: &str) -> Result<(), anyhow::Error> {
        if type_ == "answer" {
            ...
            self.pipeline.call_async(move |_pipeline| {
                ...
                app.0
                    .webrtcbin
                    .emit("create-answer", &[&None::<gstreamer::Structure>, &promise])
                    .unwrap();
            });
    ...
    fn on_ice_candidate(&self, mlineindex: u32, candidate: String) -> Result<(), anyhow::Error> {
        println!("send {} {}", mlineindex, candidate);
    ...
    fn on_incoming_stream(&self, pad: &gstreamer::Pad) -> Result<(), anyhow::Error> {
    ...
    fn on_incoming_decodebin_stream(&self, pad: &gstreamer::Pad) -> Result<(), anyhow::Error> {
